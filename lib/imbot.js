...
var imb0t = function (options) {
    var bot = {
        ...
        , handleMessage: function(from, to, message) {
            // @@@ Don't like having to reference bot here. Better way?
            bot.pattern_list.forEach(function(pattern) {
                var matches = message.match(pattern.rxp);
                if (matches) {
                    pattern.func(this.client, from, to, message, matches, bot);
                }
            }, this);
        }
        ...
        , connect: function () {
            this.connectClient(options['server'], options['nick'], options['channels']);
        }
    };
